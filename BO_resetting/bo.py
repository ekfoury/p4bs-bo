...
class BO():
    def __init__(self, length_scale = 0.0325, kernel = RBF, alpha = 0.00000001, strategy = max_EI, MAXIMUM_VAL = 200000, GRID_UNIT_SIZE = 1500): #GRID_UNIT_SIZE = 1000
        self.length_scale = length_scale
        self.kernel = kernel
        self.alpha = alpha
        self.strategy = strategy
        self.regressor = GaussianProcessRegressor(kernel=kernel(length_scale), alpha = alpha)
        #self.regressor = GaussianProcessRegressor(kernel=kernel(length_scale=length_scale, nu=2.5), alpha = alpha)
    ...
    def suggest(self, tradeoff = 1e-5):
        ...
        elif (self.strategy == max_PI):
            optimizer_type = optimizer_PI
            acq = optimizer_type(self.optimizer, self.grid.reshape(-1,1), tradeoff=tradeoff)
        elif(self.strategy == max_UCB):
            optimizer_type = optimizer_UCB
            acq = optimizer_type(self.optimizer, self.grid.reshape(-1,1), beta=tradeoff)
    ...
    def plot_termination(self, max_acqs):
        fig = plt.figure(figsize=(4, 4))
        fig.tight_layout()
        plt.xlabel('Iterations')
        plt.ylabel('Stopping')
        ...
        plt.plot(max_acqs, '--b')
        fig.show()
        fig.savefig('max_acqs.pdf', bbox_inches="tight")
